...
tutor_clase_bp = Blueprint('tutor_frameworks', __name__)
...
ARCHIVOS_CLASE = [
    "static/clases/franworks/javascrip.pdf",
    "static/clases/franworks/JavaScript.mp4",
]
...
def _subir_o_recuperar(path: str) -> str | None:
    if not os.path.exists(path):
        print(f"[frameworks] Archivo no encontrado: {path}")
        return None

    if path in _cache_archivos:
        print(f"[frameworks] Reutilizando URI cacheado: {os.path.basename(path)}")
        return _cache_archivos[path]

    print(f"[frameworks] Subiendo a Gemini: {os.path.basename(path)}")
    mime = _mime(path)
    ...
    if mime.startswith("video/"):
        ...
        while archivo_gemini.state.name == "PROCESSING" and intentos < 30:
            ...
            intentos += 1
            print(f"[frameworks] Procesando video... intento {intentos}")

        if archivo_gemini.state.name != "ACTIVE":
            print(f"[frameworks] Error: video en estado {archivo_gemini.state.name}")
            return None
...
Eres Kady, un tutor académico experto en Frameworks y JavaScript.
...
- Tienes acceso al PDF de JavaScript y el video de la clase.
...
Formato exacto:
{
  "titulo": "Test: Frameworks y JavaScript",
  "preguntas": [
    {
      "id": 1,
