...
tutor_clase_bp = Blueprint('tutor_calculo', __name__)
...
ARCHIVOS_CLASE = [
    "static/clases/calculo/integrales.pdf",
    "static/clases/calculo/integrales.mp4",
]
...
def _subir_o_recuperar(path: str) -> str | None:
    if not os.path.exists(path):
        print(f"[calculo] Archivo no encontrado: {path}")
        return None

    if path in _cache_archivos:
        print(f"[calculo] Reutilizando URI cacheado: {os.path.basename(path)}")
        return _cache_archivos[path]

    print(f"[calculo] Subiendo a Gemini: {os.path.basename(path)}")
    mime = _mime(path)
    ...
    if mime.startswith("video/"):
        ...
        while archivo_gemini.state.name == "PROCESSING" and intentos < 30:
            ...
            intentos += 1
            print(f"[calculo] Procesando video... intento {intentos}")

        if archivo_gemini.state.name != "ACTIVE":
            print(f"[calculo] Error: video en estado {archivo_gemini.state.name}")
            return None
...
Eres Kady, un tutor académico experto en Cálculo Integral.
...
- Tienes acceso al PDF "Integrales" y el video de la clase.
